...
public class OwnerDao {
    ...
    public List<Owner> buscaApellido(String apellido) {
        Query queryPets = entityManager.createQuery("select e from Owner e where e.surname = :apellido")
                .setParameter("apellido", apellido);
        return queryPets.getResultList();
    }

    public List<Owner> buscaMascotasChipPeso150() {
        // sql = "select o.* from pet p join owner o on o.id = p.owner_id where haschip
        // = true and weight > 150;"
        // Query queryPets = entityManager.createQuery("select e.owner from Pet e where
        // e.hasChip = true and e.weight > 150");
        Query queryPets = entityManager.createQuery(
                "select o from Pet as p join Owner as o  with o.id = p.owner.id where p.hasChip  = true and p.weight > 150");
        return queryPets.getResultList();
    }

    public void insertaPrevioCheck(Owner o) {
        Query consulta = entityManager
                .createQuery("select count(o) from Owner o where name = :nombre and surname  = :apellido")
                .setParameter("nombre", o.getName()).setParameter("apellido", o.getSurname());
